...
public class RegistroClientes extends javax.swing.JFrame implements Listener {
    ...
    private void itemReporteClienteActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_itemReporteClienteActionPerformed
        ...
        if (dialogo.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            ...
            boolean resultado = ControladorPrestamo.getInstancia().generarReporteClientes(ruta);
            if (resultado) {
                JOptionPane.showMessageDialog(this, "Reporte guardado en la ruta: " + ruta,
                        "Generado reporte", JOptionPane.INFORMATION_MESSAGE);
            } else {
    ...
    private void actualizarDistritos() {
        ...
        for (Distrito distrito : distritos) {
            comboDistrito.addItem(new ItemComboBox<>(distrito));
        }
    }

    private void actualizarCantones() {
        comboCanton.removeAllItems();
        List<Canton> cantones = ControladorPrestamo.getInstancia()
                .getCantones(ControladorPrestamo.getInstancia().getCodigoProvincia());
        for (Canton canton : cantones) {
            comboCanton.addItem(new ItemComboBox<>(canton));
        }
        actualizarDistritos();
    }
    ...
    private void guardar(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_guardar
        ...
        try {
            ...
            canton = ((ItemComboBox<Canton>) comboCanton.getSelectedItem()).getValor();
            distrito = ((ItemComboBox<Distrito>) comboDistrito.getSelectedItem()).getValor();
    ...
    private void buscarCliente(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_buscarCliente
        try {
            ...
            if (c != null) {
                ControladorPrestamo.getInstancia()
                        .setCodigoProvincia(CodigoProvincia.getCodigoProvincia(c.getProvincia()).getCodigo());
                txtCedula.setText(c.getId().toString());
                txtNombre.setText(c.getNombre());
                txtProvincia.setText(c.getProvincia());
                actualizarCantones();
                comboCanton.setSelectedItem(new ItemComboBox<>(c.getCanton()));
                comboDistrito.setSelectedItem(new ItemComboBox<>(c.getDistrito()));
            } else {
